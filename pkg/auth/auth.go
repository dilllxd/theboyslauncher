...
// This now uses the accounts manager for multi-account support.
func Authenticate() (Session, error) {
	// Initialize accounts manager if not already done
	if GlobalAccountsManager == nil {
		if err := InitAccountsManager(); err != nil {
			return Session{}, fmt.Errorf("initialize accounts manager: %w", err)
		}
	}

	// Try to authenticate with active account
	if GlobalAccountsManager.ActiveAccount != "" {
		return GlobalAccountsManager.AuthenticateAs(GlobalAccountsManager.ActiveAccount)
	}
...
func AuthenticateWithRedirect(success, fail string) (Session, error) {
	...
	authStore.MSA.write(resp)

	// Complete authentication to get session using temporary auth store
	originalStore := Store
	Store = authStore
	defer func() { Store = originalStore }()

	session, err := authenticateFromStore()
	if err != nil {
		return Session{}, err
	}

	// Copy the fully populated store back to authStore before saving
	authStore = Store

	// Save to accounts manager
	if GlobalAccountsManager == nil {
		if err := InitAccountsManager(); err != nil {
			return Session{}, fmt.Errorf("initialize accounts manager: %w", err)
		}
	...
	if err := GlobalAccountsManager.AddAccount(session, authStore); err != nil {
		return Session{}, fmt.Errorf("save account to accounts manager: %w", err)
	}
